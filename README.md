...
- **练习日历**（导航入口：顶部「练习日历」菜单项，路径 `/calendar`）：
  ...
  - 点击记录标题可跳转至详情页
- **牌组预设管理**（导航入口：顶部「牌组预设」菜单项，路径 `/deck-presets`）：
  - 列表页展示所有牌组预设的名称与简介，支持新建、编辑、删除单条预设
  - 支持**批量删除**：每条记录前提供复选框，表头支持全选/反选；顶部「批量删除」按钮始终显示，未选中任何记录时自动禁用；点击后弹出确认对话框，确认后一次性删除全部选中项，删除成功后页面顶部显示绿色成功提示
  - 单条删除成功后，被删除的编号会自动从当前选中集合中移除
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/deck-presets` | 预设列表（按创建顺序） |
| GET | `/api/deck-presets/:id` | 单条预设详情 |
| POST | `/api/deck-presets` | 新建预设 |
| PUT | `/api/deck-presets/:id` | 更新预设 |
